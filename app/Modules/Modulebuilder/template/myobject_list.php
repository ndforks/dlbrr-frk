...
$groupby = request()->input('groupby');	// Example: $groupby = 'p.fk_opp_status' or $groupby = 'p.fk_statut'
...
if (!empty($object->ismultientitymanaged) && (int) $object->ismultientitymanaged == 1) {
	$sql .= " WHERE t.entity IN (".getEntity($object->element, (request()->integer('search_current_entity', 0) ? 0 : 1)).")";
	...
	$sql .= " WHERE pt.entity IN (".getEntity($object->element, (request()->integer('search_current_entity', 0) ? 0 : 1)).")";
} else {
	$sql .= " WHERE 1 = 1";
}
