...
class PricecyField extends CommonField
{
	...
	{
		$htmlName = $keyPrefix . $key . $keySuffix;
		$value = request()->input($htmlName) . ':' . request()->input($htmlName . "currency_id");
	...
	public function getPostFieldValue($fieldInfos, $key, $defaultValue = null, $keyPrefix = '', $keySuffix = '')
	{
		...
		if (request()->has($htmlName)) {
			$value = price2num(request()->input($htmlName)) . ':' . request()->input($htmlName . "currency_id");
		} else {
			$value = $defaultValue;
		}
	...
	public function getPostSearchFieldValue($fieldInfos, $key, $defaultValue = null, $keyPrefix = '', $keySuffix = '')
	{
		...
		if (request()->has($htmlName)) {
			$value = array(
				...
				'currency' => request()->input($htmlName . "currency_id"),
			);
		} else {
			$value = $defaultValue;
		}
