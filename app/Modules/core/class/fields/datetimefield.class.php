...
class DatetimeField extends CommonField
{
	...
	public function getPostFieldValue($fieldInfos, $key, $defaultValue = null, $keyPrefix = '', $keySuffix = '')
	{
		...
		if (request()->has($htmlName . 'hour') || request()->has($htmlName . 'min') || request()->has($htmlName . 'sec') || request()->has($htmlName . 'month') || request()->has($htmlName . 'day') || request()->has($htmlName . 'year')) {
			$value = dol_mktime(request()->integer($htmlName . 'hour', 0), request()->integer($htmlName . 'min', 0), request()->integer($htmlName . 'sec', 0), request()->integer($htmlName . 'month', 0), request()->integer($htmlName . 'day', 0), request()->integer($htmlName . 'year', 0), 'tzuserrel'); // for date without hour, we use gmt
		} else {
			$value = $defaultValue;
		}
	...
	public function getPostSearchFieldValue($fieldInfos, $key, $defaultValue = null, $keyPrefix = '', $keySuffix = '')
	{
		...
		if (request()->has($htmlName . '_startmonth') || request()->has($htmlName . '_startday') || request()->has($htmlName . '_startyear')) {
			$start = dol_mktime(0, 0, 0, request()->integer($htmlName . '_startmonth', 0), request()->integer($htmlName . '_startday', 0), request()->integer($htmlName . '_startyear', 0), 'tzuserrel');
		} else {
		...
		if (request()->has($htmlName . '_endmonth') || request()->has($htmlName . '_endday') || request()->has($htmlName . '_endyear')) {
			$end = dol_mktime(23, 59, 59, request()->integer($htmlName . '_endmonth', 0), request()->integer($htmlName . '_endday', 0), request()->integer($htmlName . '_endyear', 0), 'tzuserrel');
		} else {
