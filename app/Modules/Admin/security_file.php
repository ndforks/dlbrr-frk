...
if ($action == 'updateform') {
	$antivircommand = request()->input('MAIN_ANTIVIRUS_COMMAND'); // Use GETPOST restricthtml because we must accept ". Example c:\Progra~1\ClamWin\bin\clamscan.exe
	$antivirparam = request()->input('MAIN_ANTIVIRUS_PARAM'); // Use GETPOST restricthtml because we must accept ". Example --database="C:\Program Files (x86)\ClamWin\lib"
	...
	if (!$error) {
		$tmpumask = request()->input('MAIN_UMASK');
		...
		$res3 = dolibarr_set_const($db, 'MAIN_UPLOAD_DOC', request()->input('MAIN_UPLOAD_DOC'), 'chaine', 0, '', $conf->entity);
		...
		$res7 = dolibarr_set_const($db, "MAIN_FILE_EXTENSION_UPLOAD_RESTRICTION", request()->input('MAIN_FILE_EXTENSION_UPLOAD_RESTRICTION'), 'chaine', 0, '', $conf->entity);

		$res8 = dolibarr_set_const($db, "MAIN_SECURITY_MAXFILESIZE_DOWNLOADED", request()->input('MAIN_SECURITY_MAXFILESIZE_DOWNLOADED'), 'chaine', 0, '', $conf->entity);
	...
	// Delete file
	$langs->load("other");
	$file = $conf->admin->dir_temp.'/'.request()->input('urlfile');
...
if (getDolGlobalString('MAIN_ANTIVIRUS_UPLOAD_ON')) {
	...
	print '<input type="text" '.((defined('MAIN_ANTIVIRUS_COMMAND') && !defined('MAIN_ANTIVIRUS_BYPASS_COMMAND_AND_PARAM')) ? 'disabled ' : '').'name="MAIN_ANTIVIRUS_COMMAND" class="minwidth500imp" spellcheck="false" value="'.dol_escape_htmltag(request()->has('MAIN_ANTIVIRUS_COMMAND') ? request()->input('MAIN_ANTIVIRUS_COMMAND') : getDolGlobalString('MAIN_ANTIVIRUS_COMMAND')).'">';
