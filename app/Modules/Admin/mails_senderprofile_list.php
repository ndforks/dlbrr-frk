...
$contextpage = request()->input('contextpage') ? request()->input('contextpage') : 'emailsenderprofilelist'; // To manage different context of search
...
if (empty($reshook)) {
	...
	if ($action == 'delete') {
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."c_email_senderprofile WHERE rowid = ".request()->integer('id', 0);
		$resql = $db->query($sql);
		if ($resql) {
...
//if (request()->input('nomassaction') || in_array($massaction, array('presend', 'predelete'))) $arrayofmassactions = array();
...
if ($action != 'create') {
	...
	if ($action == 'edit') {
		...
		print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td><input type="text" name="label" class="width300" value="'.(request()->has('label') ? request()->input('label') : $object->label).'"></td></tr>';
		print '<tr><td>'.$langs->trans("Email").'</td><td>';
		print img_picto('', 'email', 'class="pictofixedwidth"');
		print '<input type="text" name="email" value="'.(request()->has('email') ? request()->input('email') : $object->email).'"></td></tr>';
		print '<tr><td>'.$langs->trans("Signature").'</td><td>';
		require_once DOL_DOCUMENT_ROOT.'/Core/class/doleditor.class.php';
		$doleditor = new DolEditor('signature', (request()->has('signature') ? request()->input('signature') : $object->signature), '', 138, 'dolibarr_notes', 'In', true, true, !getDolGlobalString('FCKEDITOR_ENABLE_USERSIGN') ? 0 : 1, ROWS_4, '90%');
		print $doleditor->Create(1);
		print '</td></tr>';
		print '<tr><td>'.$langs->trans("User").'</td><td>';
		print img_picto('', 'user', 'class="pictofixedwidth"');
		print $form->select_dolusers((request()->has('private') ? request()->integer('private', 0) : $object->private), 'private', 1, null, 0, ($user->admin ? '' : $user->id));
		print '</td></tr>';
		print '<tr><td>'.$langs->trans("Position").'</td><td><input type="text" name="position" class="maxwidth50" value="'.(request()->has('position') ? request()->integer('position', 0) : $object->position).'"></td></tr>';
		print '<tr><td>'.$langs->trans("Status").'</td><td>';
		print $form->selectarray('active', $object->fields['active']['arrayofkeyval'], (request()->has('active') ? request()->integer('active', 0) : $object->active), 0, 0, 0, '', 1);
		print '</td></tr>';
		print '</table>';
	...
	*/
	print '<table class="border centpercent tableforfield">';
	print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td><input type="text" name="label" class="width300" value="'.request()->input('label').'" autofocus></td></tr>';
	print '<tr><td class="fieldrequired">'.$langs->trans("Email").'</td><td>';
	print img_picto('', 'email', 'class="pictofixedwidth"');
	print '<input type="text" name="email" class="width300" value="'.request()->input('email').'"></td></tr>';
	print '<tr><td>'.$langs->trans("Signature").'</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/Core/class/doleditor.class.php';
	$doleditor = new DolEditor('signature', request()->input('signature'), '', 138, 'dolibarr_notes', 'In', true, true, !getDolGlobalString('FCKEDITOR_ENABLE_USERSIGN') ? 0 : 1, ROWS_4, '90%');
	print $doleditor->Create(1);
	print '</td></tr>';
	print '<tr><td>'.$langs->trans("User").'</td><td>';
	print img_picto('', 'user', 'class="pictofixedwidth"');
	print $form->select_dolusers((request()->has('private') ? request()->integer('private', 0) : -1), 'private', 1, null, 0, ($user->admin ? '' : $user->id));
	print '</td></tr>';
	print '<tr><td>'.$langs->trans("Position").'</td><td><input type="text" name="position" class="maxwidth50" value="'.request()->integer('position', 0).'"></td></tr>';
	print '<tr><td>'.$langs->trans("Status").'</td><td>';
	print $form->selectarray('active', $object->fields['active']['arrayofkeyval'], request()->integer('active', 0), 0);
	print '</td></tr>';
	print '</table>';
