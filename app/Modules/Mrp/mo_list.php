...
$year   = request()->integer('change_dateyear', 0);
$month   = request()->input('change_datemonth');
$day   = request()->input('change_dateday');
$hour   = request()->input('change_datehour');
$min   = request()->input('change_datemin');
...
if (request()->has('formfilteraction')) {
	$searchCategoryMoOperator = request()->integer('search_category_mo_operator', 0);
} elseif (getDolGlobalString('MAIN_SEARCH_CAT_OR_BY_DEFAULT')) {
	$searchCategoryMoOperator = getDolGlobalString('MAIN_SEARCH_CAT_OR_BY_DEFAULT');
}
$searchCategoryMoList = request()->input('search_category_mo_list');
$search = array();
foreach ($object->fields as $key => $val) {
	if ($key == 'status' && request()->has('search_status')) {
		$search_status = request()->input('search_status');
		if (empty($search_status) && request()->input('search_status') !== '') {
			$search_status = request()->input('search_status');
			if ($search_status !== -1) {
