...
if (empty($reshook) && $action == 'add') {	// Test on permission not required here. This is an anonymous public ssubmission. Check is done on the secureket + mitigation.
	...
	if (!request()->input('country_id') && !empty((float) $project->price_booth)) {
		$error++;
...
print '</tr>';
// Email
print '<tr><td>'.$langs->trans("Email").'<span class="star">*</span></td><td><input type="text" name="email" maxlength="255" class="minwidth150" value="'.dol_escape_htmltag(request()->input('email')).'"></td></tr>'."\n";
// Company
...
print '</td><td><input type="text" name="label" class="minwidth150" value="'.dol_escape_htmltag(request()->input('label')).'"></td></tr>'."\n";
// Note
print '<tr><td>'.$langs->trans("Description").'<span class="star">*</span></td>'."\n";
print '<td><textarea name="note" id="note" wrap="soft" class="quatrevingtpercent" rows="'.ROWS_3.'">'.dol_escape_htmltag(request()->input('note'), 0, 1).'</textarea></td></tr>'."\n";
