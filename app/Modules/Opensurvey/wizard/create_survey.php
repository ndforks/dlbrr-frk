...
$mailsonde = request()->input('mailsonde');
$creation_sondage_date = request()->input('creation_sondage_date');
$creation_sondage_autre = request()->input('creation_sondage_autre');
...
if (!empty($creation_sondage_date) || !empty($creation_sondage_autre)) {
	...
	$testdate = false;
	$champdatefin = (int) dol_mktime(23, 59, 59, request()->integer('champdatefinmonth', 0), request()->integer('champdatefinday', 0), request()->integer('champdatefinyear', 0));
...
if (!$_SESSION["title"] && (request()->input('creation_sondage_date') || request()->input('creation_sondage_autre'))) {
...
if (request()->has('allow_comments')) {
	$allow_comments = request()->input('allow_comments') ? 'checked' : '';
}
...
if (request()->has('allow_spy')) {
	$allow_spy = request()->input('allow_spy') ? 'checked' : '';
}
...
if (request()->input('choix_sondage')) {
	if (request()->input('choix_sondage') == 'date') {
	...
	print '<input type="hidden" name="choix_sondage" value="'.request()->input('choix_sondage').'">';
	print '<br><input type="submit" class="button" name="submit" value="'.$langs->trans("CreatePoll").' ('.(request()->input('choix_sondage') == 'date' ? $langs->trans("TypeDate") : $langs->trans("TypeClassic")).')">';
} else {
